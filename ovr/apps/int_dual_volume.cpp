...
public:
  MainWindow(const Args& commandline, const std::string& title, Camera camera, const float worldScale)
    : GLFCameraWindow(title, camera.from, camera.at, camera.up, worldScale, 1024 * 2, 1024)
  ...
  void gui()
  {
    ...
    if (ImGui::Begin("Control Panel", NULL)) {
      ...
      const char* render_modes = " 0 Ray Marching (No Shading)  \0"
                                 " 1 Ray Marching (Gradient)    \0"
                                 " 2 Ray Marching (Single-Shot) \0"
                                 " 3 Path Tracing               \0";
