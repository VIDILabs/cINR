...
public:
  ...
  void render_background()
  {
    ...
    if (background_fps.count()) {
      ...
      frame_time = 0.0;
      float fps_avg = std::accumulate(background_fps.fps_history.begin(), background_fps.fps_history.end(), 0.0) / background_fps.fps_history.size();
      std::cout << "Avg FPS: " << fps_avg << " \r";
    }
  }
  /* GUI thread*/
  // void move() 
  ...
  //   frame++;
  //   if (frame >= 5) {
  //     if (abs(config.rot) > 0 && rot_angle < 90) {
  //       cameraFrame.rotate_frame_by_angle(cos(config.axis*(M_PI/180)) * config.rot, sin(config.axis*(M_PI/180)) * config.rot);
  //       cameraFrame.modified = true;
  //       rot_angle += abs(config.rot);
  //       frame = 0;
  //     }
  //     else if (rot_angle >= 90) {
  //       rot_angle -= 90;
  //       frame = -1 * config.pause * 100;
  //     }
  //     if (zoom_lvl != config.zoom) {
  //       // TODO: Enable auto zoom mode
  //       int step = 1;
  //       if (zoom_lvl < config.zoom)
  //         zoom_lvl++;
  //       else {
  ...
  //         step = -1;
  //       }
  //       vec3f old_pos = cameraFrame.get_position();
  //       cameraFrame.set_position(cameraFrame.get_position()*vec3f(1.1, 1.1, 1.1)*step);
  //       cameraFrame.set_focal_length(length(cameraFrame.get_position() - old_pos));
  ...
  void record(std::string suffix="")
  {
    vnr::CacheConfig* user_data = (vnr::CacheConfig*)renderer->current_scene.user_data;
    int pos = user_data->config.rfind('/');

    std::string name = user_data->config.substr(pos+8, user_data->config.length()-pos-12);
    // float fps_avg = std::accumulate(background_fps.fps_history.begin(), background_fps.fps_history.end(), 0.0) / background_fps.fps_history.size();
    if (suffix == "Clear") {
      std::ofstream fileInit(name + "_" + mode + ".csv", std::ios::trunc);
      fileInit << name << ',' << "time" << '\n';
      fileInit.close();
      return;
    }
    ...
    std::chrono::duration<float> time_eclipsed = std::chrono::duration_cast<std::chrono::duration<float>>(now - start);
 
    std::ofstream file(name + '_' + suffix + ".csv", std::ios::app);

    file << background_fps.fps << ',' << time_eclipsed.count() << '\n';

    file.close();
  }
  
  void record_controller() 
  {
    const int end = max_frame_count;
    ...
    if (frame == 0) {
      ...
      renderer->set_path_tracing((mode == "PT"));
      record("Clear");
    }
    ...
    if (frame == end) {
      std::cout << mode + " Recording Complete\n\n";
    ...
    // if (frame == 0) {
    //   renderer->set_path_tracing(true);  // enable for PT
    //   record("ClearPT");
    // }

    // if (frame == end) {
    //   std::cout << "PT Recording Complete\n\n";
    //   std::exit(0);
    // }

    // if (frame < end) // Path tracing record
    // {
    //   record("PT");
    // }
  ...
  void draw() override
  {
    ...
    if (gui_enabled) {
      ...
      // renderer->current_scene.net_id // TODO: here net_id, recreate device with new id to load new net

      // Movement UI
      // ImGui::SetNextWindowSizeConstraints(ImVec2(450, 400), ImVec2(FLT_MAX, FLT_MAX));
      // if (ImGui::Begin("Movement Panel", NULL)) {
      //   static int rot = config.rot;
      //   if(ImGui::SliderInt("Rotation Speed", &rot, -10, 10)) {
      //     config.rot = rot;
      //   }
      //   static int pause = config.pause;
      //   if(ImGui::SliderInt("Pause Length (100fr)", &pause, 0, 10)) {
      //     config.pause = pause;
      //   }
      //   static int axis = config.axis;
      //   if(ImGui::SliderInt("Rotation Axis (deg)", &axis, 0, 180)) {
      //     config.axis = axis;
      //   }
      //   static int zoom = config.zoom;
      //   if(ImGui::SliderInt("Zoom Level", &zoom, -100, 100)) {
      //     config.zoom = zoom;
      //   }
    ...
    if (foreground_fps.count()) {
      ...
      title << std::fixed << std::setprecision(1) << " bg = " << background_fps.fps << " fps,";
