...
inline __device__ bool
intersectVolume(float& _t0, float& _t1, const vec3f ray_org, const vec3f ray_dir, const VNRDeviceVolume& volume)
{
...
template<typename T, typename Array1D>
static __device__ T
array1dNodal(const Array1D& array, float v)
{
  if (array.dims.v == 0) return T{0};
  assert(array.dims.v > 0 && "invalid array size");
  v = clamp(v, 0.f, 1.f);
  const float t = fmaf(v, float(array.dims.v-1), 0.5f) * __frcp_rn(array.dims.v);
...
sampleVolume(const Array3DScalarCUDA& self, vec3f p)
{
...
sampleGradient(const Array3DScalarCUDA& self,
...
{
  const auto v = (clamp(sampleValue, tfn.value_range.lower, tfn.value_range.upper) - tfn.value_range.lower) * tfn.range_rcp_norm;
...
static __device__ void
opacityCorrection(const VNRDeviceVolume& self, const float& distance, float& opacity)
{
  opacity = 1.f - __powf(1.f - opacity, self.step_rcp * distance);
}

inline __device__ float
opacityUpperBound(const VNRDeviceVolume& self, const vec3i& cell)
{
  ...
  assert(cell.y >= 0);
  assert(cell.z >= 0);
  // printf("id: %u | Max_Opacity: %f \n\n", idx, self.macrocell_max_opacity[idx]);
