...
struct DDAIter
{
  ...
  bool __device__ next(vec3f org, vec3f dir, float t_min, float t_max, vec3i grid_size, bool dbg, const Lambda& lambda)
  {
    ...
    // MAYBE_UNUSED const float& ray_t0=t_min;
    // MAYBE_UNUSED const float& ray_t1=t_max;
    assert(t_min < t_max);
...
dda3(vec3f org, vec3f dir, float t_min, float t_max, vec3i grid_size, bool dbg, const Lambda& lambda)
{
  ...
  // if (dbg) printf("Org vector: %f %f %f\n", org.x, org.y, org.z);
  // if (dbg) printf("t range for volume %f %f\n",ray_t0,ray_t1);
  // if (dbg) printf("Dir vector: %f %f %f\n", dir.x, dir.y, dir.z);
  // if (dbg) printf("Ray t0 & t1: %f %f\n", ray_t0, ray_t1);
  ...
  // if (dbg) printf("Floor out: %f %f %f\n",floor(org_in_volume.x),floor(org_in_volume.y), floor(org_in_volume.z));
  // if (dbg) printf("grid_size: %i %i %i\n", grid_size.x, grid_size.y, grid_size.z);
  ...
  while (true) {
    vec3f f_cell_end = {
    dir.x > 0.f ? cell.x + 1.f : cell.x,
    dir.y > 0.f ? cell.y + 1.f : cell.y,
    dir.z > 0.f ? cell.z + 1.f : cell.z
    };
    vec3f t_next = { ((dir.x == 0.f) ? float_large : (abs(f_cell_end.x - org_in_volume.x) * t_step.x)),
                   ((dir.y == 0.f) ? float_large : (abs(f_cell_end.y - org_in_volume.y) * t_step.y)),
                   ((dir.z == 0.f) ? float_large : (abs(f_cell_end.z - org_in_volume.z) * t_step.z)) };
