...
update_single_macrocell(const vec3i& voxel, const vec3i& dims, float* __restrict__ macrocells, float value)
{
  ...
  // atomicMin(vmin, value);
  // atomicMax(vmax, value);
}
                             ...
                             cudaTextureObject_t volumeTexture)
{
  ...
  vec3i end   = min(begin + vec3i(mcWidth) + /* plus one for tri-lerp!*/ 1, volumeDims);
                             ...
                             float* __restrict__ cell_max_opacity)
{
  ...
  for (auto i = i_lower; i <= i_upper; ++i) {
    ...
    // if (threadIdx.x + blockIdx.x * blockDim.x == 50)
    // printf("alpha[%i]: %f | Opacity: %f\n\n", i, alphas[i], opacity);
  }
  // printf("a0: %f | a1: %f\n\n", alphas[0], alphas[1]);
  // if (i == 52198)
  //   printf("i: %u | i_lower: %u | i_upper: %u | lower: %f | upper: %f | tfn.lower: %f | tfm.upper: %f | Range: [%f, %f] | Opacity: %f | rcp: %f | alpha-len: %f \n\n", i, i_lower, i_upper, lower, upper, tfn.range.lower, tfn.range.upper, range.x, range.y, opacity, tfn.range_rcp_norm, float(tfn.alphas.length));
...
void MacroCell::update_max_opacity(const DeviceTransferFunction& tfn, cudaStream_t stream) 
{
  ...
  // std::cout << m_dims << ' ' << (uint32_t)tfn.alphas.length << ' ' << (uint32_t)shmem << std::endl;
